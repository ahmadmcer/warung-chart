...
export default function IncomeScreen() {
  ...
  const [valueIncome, setValueIncome] =
    useState(0);
  ...
  const saveAmount = async () => {
    try {
      ...
      if (!tempData) {
        await db.execAsync(
          `INSERT INTO wallet (date, income, outcome) VALUES ('${date}', ${valueIncome}, 0)`
        );
      } else {
        await db.execAsync(
          `UPDATE wallet SET income = ${valueIncome} WHERE date = '${date}'`
        );
      }
    } catch (error) {
      console.error(error);
